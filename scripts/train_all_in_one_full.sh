#!/usr/bin/env bash
# Strategy A (full) — one training pass on BOTH primitive and downstream tasks together.
# The trainer auto-includes every contract folder under data/ when --training_strategy
# is all_in_one (the family catalog is not utilized on the supervision side; image loss
# only; no BCE gate loss). Override TRAIN_DATASET_IDS / EXCLUDE_DATASET_IDS to pin or
# trim the pool. See docs/architecture/training_strategy.md.
...
RUN_TAG="${RUN_TAG:-all_in_one_full}"
